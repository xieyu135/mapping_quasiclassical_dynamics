...
exe_spec="$HOME/program/spectral_density/Debye/main_spec_dens.exe"
exe_gener_lambda0="python $main_dir/gener_lambda0_coupling.py"
exe_gener_lambda1="python $main_dir/gener_lambda1_coupling.py"
exe_gener_input_files="python $HOME/program/mapping/scripts/gener_input_files_2model.py"
...
do
  ...
  do
    ...
    E_re=`awk -v a=$kdw -v w=$wc 'BEGIN{print a^2*w}'`
    echo "mkdir $work_dir"
    mkdir -p $work_dir
