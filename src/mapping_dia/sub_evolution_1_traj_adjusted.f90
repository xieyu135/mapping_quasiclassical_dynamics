subroutine sub_evolution_1_traj_adjusted
use mod_main
implicit none
...
real(kind=8), allocatable, dimension(:) :: gammas
...
call sub_cal_gammas_1(x_elec, y_elec, p_x_elec, p_y_elec, gammas)
!
...
if (label_aver_PES == 1) then
  ...
  !     write(*,*) 'sub_get_dia_Ham_elec done!'
       !write(*,*) exci_e
       call sub_write_trj_1_traj_adjusted(i_step, &
                          x_cur, p_cur, &
                          x_elec, y_elec, &
                          p_x_elec, p_y_elec, &
                          gammas, &
  ...
  !         if(label_diabatic == 1)then
     call sub_elec_motion_1_aver_PES(x_cur, p_cur, &
                          x_pre, p_pre, &
                          x_elec, y_elec, &
                          p_x_elec, p_y_elec, &
                          freq, k_tun, exci_e, &
  ...
  !   write(*,*) 'sub_elec_motion done'
     call sub_get_c_n_elec_1_traj_adjusted(x_elec, y_elec, &
                           p_x_elec, p_y_elec, &
                           gammas, &
                           c_n)
  ...
  !         if(label_diabatic == 1)then
     call sub_elec_motion_1(x_cur, p_cur, &
                          x_pre, p_pre, &
                          x_elec, y_elec, &
                          p_x_elec, p_y_elec, &
                          freq, k_tun, exci_e, &
