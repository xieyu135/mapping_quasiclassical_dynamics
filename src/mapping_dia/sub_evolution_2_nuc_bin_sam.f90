subroutine sub_evolution_2_nuc_bin_sam
use mod_main
implicit none
...
if (label_aver_PES == 1) then
  ...
  !   write(*,*) i_step
     if (mod(i_step,n_save_trj) .eq. 0 ) then
       ...
       call sub_write_trj_2(i_step, &
                          ...
                          x_elec, p_elec, &
                          Ham)
       call  sub_write_ene_symm_2(i_step, &
                         x_cur, p_cur, &
                         x_elec, p_elec, &
                         Ham)
     endif
  ...
  !   write(*,*) 'sub_elec_motion done'
     ...
     x_pre = x_cur
     p_pre = p_cur
     call sub_nuc_motion_aver_PES_bin_sam( x_cur, p_cur, &
                          c_n, &
                          freq, k_tun, exci_e, &
                          lambda0, lambda1)
  ...
  stop 'no_aver_PES version is not available.'
  do i_step=0, n_step
  ...
  !   write(*,*) i_step
     if (mod(i_step,n_save_trj) .eq. 0 ) then
       ...
       call sub_write_trj_2(i_step, &
                          ...
                          x_elec, p_elec, &
                          Ham)
       call sub_write_ene_nosymm_2(i_step, &
                          x_cur, p_cur, &
                          x_elec, p_elec, &
                          Ham)
