subroutine sub_write_ene_nosymm_2(i_step, &
                         x_cur, p_cur, &
                         x_elec, p_elec, &
                         Ham)
use mod_main
...
! --- local variables ---
real(kind=8) ene_all, ene_nuc_kin, time
integer i_state, j_state, i_mode

ene_all = 0.d0
do i_state = 1, n_state
    ene_all = (x_elec(i_state)**2 + p_elec(i_state)**2 - ggamma) * &
              Ham(i_state, i_state) + ene_all
enddo
