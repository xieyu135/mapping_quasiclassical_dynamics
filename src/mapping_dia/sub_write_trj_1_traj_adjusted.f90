subroutine sub_write_trj_1_traj_adjusted(i_step, &
                           x_cur, p_cur, &
                           x_elec, y_elec, &
                           p_x_elec, p_y_elec, &
                           gammas, &
...
real(kind=8), dimension(n_state) :: x_elec, y_elec, gammas
...
!write(*,*) mat_tmp
do i = 1, n_state
  mat_pop_dia(i,i) =  x_elec(i) * p_y_elec(i) - y_elec(i) * p_x_elec(i) - gammas(i)
enddo
...
do i = 1, n_state-1
  do j = i+1, n_state
    if (i == j) cycle
    tmp = (mat_pop_dia(i,j) + mat_pop_dia(j,i)) * 0.5d0
    mat_pop_dia(i,j) = tmp
    mat_pop_dia(j,i) = tmp
  enddo
enddo
