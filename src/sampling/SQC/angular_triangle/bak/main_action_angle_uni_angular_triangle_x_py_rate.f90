program main_action_angle_uni_angular_triangle_x_py_rate
implicit none
! by Yu Xie. Email: xieyu135@sina.com
include 'param.def'
integer, parameter :: SQRT2 = 2.d0**0.5d0
integer n_state
integer n_traj
character(len=72) filename_ini_n
character(len=72) filename_x_p_traj
real(kind=8) x_py_rate_max
...
do i = 1, n_traj
  x_tmp = (random_1(i, i_occ) - 0.5d0) * SQRT2
  p_x_tmp = (random_2(i, i_occ) - 0.5d0) * SQRT2
  y_tmp = (random_3(i, i_occ) - 0.5d0) * SQRT2
  p_y_tmp = (random_4(i, i_occ) - 0.5d0) * SQRT2
...
do i = 1, n_traj
  do j = 1, n_state
    if (j == i_occ) cycle
    x_tmp = (random_1(i, j) - 0.5d0) * SQRT2
    p_x_tmp = (random_2(i, j) - 0.5d0) * SQRT2
    y_tmp = (random_3(i, j) - 0.5d0) * SQRT2
    p_y_tmp = (random_4(i, j) - 0.5d0) * SQRT2
    tmp = x_tmp * p_y_tmp - y_tmp * p_x_tmp
    if (tmp < 0) then
      x_tmp = - x_tmp
      y_tmp = - y_tmp
    ...
    aaction = random_5(i, j) * (2.d0 - arr_n(i, i_occ))
    arr_n(i, j) = aaction
    tmp = (aaction / tmp)**0.5d0
    x_tmp = x_tmp * tmp
    y_tmp = y_tmp * tmp
